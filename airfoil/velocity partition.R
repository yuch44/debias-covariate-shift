...
thrs <- quantile(dat.x[,4], 0.5)
ind <- which(dat.x[,4]<=thrs)
x1 <- dat.x[ind,-4]; y1 <- dat.y[ind]
x2 <- dat.x[-ind,-4];y2 <- dat.y[-ind]
n1 <- dim(x1)[1]; n2 <- N-n1
...
for(i in 1:M) {
  ...
  # sample split
  split0_ind <- sample(seq_len(nrow(data0)),size=floor(0.25 * nrow(data0)))
  split1_ind <- sample(seq_len(nrow(data1)),size=floor(0.25 * nrow(data1)))
...
for(i in 1:M) {
  ...
  for(j in 1:numFolds){
    for(k in 1:numFolds){
      ...
      as <- a_outer(est_data0,est_data1)
      gamma0 <- gamma(est_data0)
      alpha0 <- predict(alpha_model,est_data0[,1:4],onlySL = T)$pred
      alpha1 <- predict(alpha_model,est_data1[,1:4],onlySL = T)$pred
      for(u in 1:(length(fold0[[j]]))){
        for(v in 1:(length(fold1[[k]]))){
          store_values[fold0[[j]][u],fold1[[k]][v]] <- gamma0[u]*as[u,v] + alpha1[v]-gamma0[u]*alpha0[u]
        }
